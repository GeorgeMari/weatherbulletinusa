...
*/
	...
	function NWSSuccess(inSender, inResponse) {
	...
	function nMDataHandler(transaction, results) {
	...
	function NWSFailure(inSender, inResponse) {
	...
	function wbPushDashboard(inText, zoneList) {
	...
	function wbPopDashboard() {
		this.$.wbDashboard.pop();
	}

	function wbMessageTap(inSender, layer) {
	...
	function wbIconTap(inSender, layer) {
		enyo.log("wbDashboard - tapped on icon for message: " + layer.text);
	}

	function wbDashboardClose(inSender) {
		enyo.log("Closed wbDashboard.");
	}

	function wbLayerSwiped(inSender, layer) {
